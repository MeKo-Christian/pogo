...
- [ ] Achieve >90% code coverage for core components (1/4 complete, 3/4 substantial progress):
  - [~] **Detection post-processing algorithms** (86.1% ↑ - closest to target)
    ...
    - [x] Adaptive thresholding (97-100% coverage): Otsu, bimodality, dynamic thresholds
    - [x] Model management (100% coverage): UpdateModelPath (100%), DetectRegions (0%)
    - [x] Post-processing variants: PostProcessDBWithNMS (100% ↑), PostProcessDBWithOptions (85%)
  - [~] **Recognition decoding logic** (77.1%)
    - [x] CTC decoding (90%+ coverage): CTCCollapse (93%), DecodeCTCGreedy (91%)
    - [x] Charset management (80-100% coverage): dictionary loading, character mapping
    - [x] Text processing (88%+ coverage): normalization, cleaning, validation
    - [x] Core inference pipeline (90%+ coverage)
    - [x] Language detection (75-82% coverage): DetectLanguage, determineLanguage
    - [ ] Model lifecycle: warmup, configuration getters (0-73% coverage)
  - [~] **Pipeline orchestration** (57.7% ↑ - significant ProcessImagesContext improvement)
    ...
    - [ ] **Core processing functions** (ProcessImagesContext major improvement ✅):
      - [ ] ProcessImagesParallelContext (25%) - parallel image processing
      - [x] ProcessImagesContext (54.5%) - context-aware processing
